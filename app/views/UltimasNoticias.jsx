...
import { db } from '../services/firebaseConfig'; // Certifique-se que este caminho está correto e 'db' é exportado
...
export default function ManchetesScreen() {
  ...
  const carregarManchetes = async () => {
    try {
      setError('');
      setLoading(true);
      // Query para manchetes: mais recentes, com limite.
      // Poderia ser ajustada para orderBy('pontuacao', 'desc') ou where('status', '==', 'publicado') etc.
      const q = query(
        collection(db, 'noticias'),
        orderBy('criadoEm', 'desc'),
        limit(10) // Carrega as 10 manchetes mais recentes
      );
      ...
      querySnapshot.forEach((doc) => {
        ...
        manchetesData.push({
          ...
          resumo: dados.conteudo || '', // Usando o conteúdo como resumo
          imagemUrl: dados.imagemUrl || dados.imagem || 'https://placehold.co/600x400/f0f2f5/c8102e?text=Notícia&font=roboto', // Placeholder
          criadoEm: dados.criadoEm?.toDate() || new Date(),
          autorNome: dados.autorNome || 'Redação', // Default para autor
          // Mantendo outros campos para consistência, caso precise no futuro
  ...
  function renderMancheteItem({ item }) {
    return (
      <TouchableOpacity
        ...
        onPress={() => router.push({
          pathname: '/views/TelaNoticiaDetalhe', // Ou o caminho correto para sua tela de detalhes
  ...
  return (
    <View style={styles.container}>
      <Text style={styles.tituloPrincipal}>ULTIMAS NOTICIAS</Text>
      <FlatList
        data={manchetes}
        ...
        refreshControl={
          <RefreshControl
            ...
            onRefresh={onRefresh}
            colors={['#C8102E']} // Cor do spinner do refresh
            tintColor="#C8102E"
          />
        }
        ListEmptyComponent={
      ...
      {usuarioAdmin && (
        <TouchableOpacity
          ...
          onPress={() => router.push('/views/TelaPublicarNoticia')} // Ou o caminho correto
        >
...
// Estilos adaptados para ManchetesScreen
const styles = StyleSheet.create({
  ...
  tituloPrincipal: {
    fontSize: 26,
    fontWeight: 'bold',
    color: '#C8102E', // Cor principal do tema
    paddingHorizontal: 16,
    paddingTop: 20, // Mais espaço no topo
    paddingBottom: 12, // Espaço abaixo do título
  ...
  imagemManchete: {
    width: '100%',
    height: 200, // Altura da imagem da manchete
  ...
  infoManchete: {
    fontSize: 12,
    color: '#6c757d', // Cinza para informações adicionais
  },
  emptyContainer: {
    flex: 1,
    justifyContent: 'center',
  ...
  botaoNovaNoticia: {
    ...
    right: 20,
    bottom: 20,
    backgroundColor: '#C8102E', // Cor principal
    width: 56,
    height: 56,
    borderRadius: 28,
    justifyContent: 'center',
